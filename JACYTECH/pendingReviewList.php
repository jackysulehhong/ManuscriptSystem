...
function getReview($conn)
{
  ...
  return query($conn, "SELECT manuscript.*,review.status AS 'reviewStatus',review.id AS 'reviewID' FROM `manuscript` JOIN `review` ON manuscript.id=review.manuscriptID WHERE review.reviewerID='$reviewerID' AND review.status='pending'");
}
  ...
  <table class="table table-hover table-light" id="tablelist">
   ...
   <tbody>
    <?php
     ...
     <td class="text-center"><button class="btn btn-primary" type="button"><a href="reviewManuscript.php?id=' . $row['reviewID']  . '" class="text-white text-decoration-none">View More</a>
     </td>
    </tr>';
        }
      }
